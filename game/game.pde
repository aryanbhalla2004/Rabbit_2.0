...
Minim minim;
AudioPlayer player;
// x and y for my animal
int x,y;
// w and h for my animal
int w,h;
PImage startscreen; 
boolean reset = false;
//BACKGROUND
...
void draw() {
    ...
    // or it will stay at stage 1
     ...
     if( y  >= 710) {
      ...
      // rabit win stage
      if(x >= 2420){
        // background for my rabit win text
        background(0);
        // text rabit win color size 
        fill(255);
        textSize(200);
        text("RABBIT WIN" ,1390,1200);
        fill(255);
        // restart button
        rectMode(CENTER);
        rect(1305,910,450,150);
        // restart text for button
        textSize(100);
        fill(255,0,0);
        text("Restart",1300,955);
        // button function and button coordinates
        if(mousePressed == true){
          if(mouseX > 885 && mouseX < 1330 && mouseY > 780 && mouseY < 930)
          {
             stage = 5;
          }
